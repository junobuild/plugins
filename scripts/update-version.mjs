...
const updateVersion = async () => {
  if (process.argv.length !== 3) {
    console.log('Invalid arguments: node update-version.mjs core|analytics|etc.');
    return;
  }

  const project = process.argv[2];

  const packagePath = join(process.cwd(), 'plugins', project, 'package.json');
  ...
  // Peer dependencies need to point to wip references - e.g. @junobuild/utils@0.0.1-next
